//! Verifiable evidence packages (`SPEC.md` §4.4, §4.5): `LogBundle`
...
//! `verify` exists to ask (`SPEC.md` §7.2's independent-replay requirement).
...
impl LogBundle {
    ...
    /// `verify` reports (`SPEC.md` §6.1), not a format error: this
    ...
    /// error here, unlike [`crate::codec::decode_entry`] (`SPEC.md`
    /// §5.1). Rejects an observer or author `NodeId` that is not strictly
    /// ascending within its list — same canonicity reasoning as
    /// `VersionVector` (§5.1): two different byte strings must never
    /// decode to the same `LogBundle`.
    pub fn decode(bytes: &[u8]) -> Result<Self, DecodeError> {
        if bytes.len() < BUNDLE_DOMAIN_TAG.len()
            || &bytes[..BUNDLE_DOMAIN_TAG.len()] != BUNDLE_DOMAIN_TAG
        {
    ...
    /// `VersionVector::merge` (`DESIGN.md` D-004) — the name collision
...
impl Spec {
    /// Canonical encoding (`SPEC.md` §4.5):
    ///
    /// ```text
    ...
    /// ```
    ///
    /// **Not signed in Phase 1** (`SPEC.md` §4.5): the verifier
