...
//! state.
//!
//! The oracle is kept, not deleted, for two reasons that have nothing to do
//! with normativity:
//!
//! - **`verify`'s differential partner.** Two independent implementations
//!   of the same predicate, where agreement is evidence and disagreement is
//!   a bug in one of them (`crates/swarm-sim/tests/m7_equivalence.rs`).
//! - **The negative control's host.** `mutant-i3` breaks
//!   `swarm_core::state::Claims::winner` itself; the oracle calls that
